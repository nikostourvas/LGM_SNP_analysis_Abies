...
```{r include=T fig.width = 2.5, fig.height=4} 
uHe2 <- uHe[-nrow(uHe),] %>%
...
ggsave(filename = "../results/figs/Abies_Gene-diversity-SNP.png", 
...
ggsave(filename = "../results/figs/Abies_Gene-diversity1-SNP.png", 
...
```{r include=T}
Fis %>% 
ggplot(aes(x=population, y=Fis, color=population)) +
...
ggsave(filename = "../results/figs/Abies_FIS-SNP.png", 
       width = 4, height = 4, units = "in", dpi=300)
```
...
p_na <- ggplot(all, aes(x=population, y=count, fill=population) ) +
  geom_bar(stat = "identity", 
           color = "black",
           width = .7) +
  geom_text(aes(label=count, vjust = -0.2, size=2)) +
...
ggplot(private) + geom_tile(aes(x = population,
  ...
  xlab("Population") + ylab("Allele")
  
}

ggsave(filename = "../results/figs/Abies_PA_SNP.png",
...
Total N of Alleles and Private alleles per Country
```{r dpi=300, include=T}
setPop(obj) <- ~Country
div <- summary(obj)
...
ggplot(all_tidy, aes(x=population, y=value, fill=type) ) +
  geom_bar(stat = "identity", position = position_dodge(),
           color = "black",
           width = .8) +
  # geom_text(aes(label=value, vjust = 1.6, size=1)) +
...
Significant LD was detected on 113 loci for the German cohort, 78 loci for the Greek cohort, and 83 loci for the Slovenian cohort.

Nine pairs of SNPs (one on different contigs, eight on the same contig), exhibited significant LD across all cohorts.

### Hardy - Weinberg equilibrium
```{r HWE, fig.width=9, fig.height=20, dpi=300, cache=TRUE, include=T}
...
### Hierarchical AMOVA - within variation included (as calculated using Arlequin)
```{r amova_arlequin2, cache=T, include=T}
### Hierarchical AMOVA - within variation included (as calculated using Arlequin)
amova_hf <- poppr.amova(obj, ~Species/Country/Pop, within = T)
amova_hf
poppr.amovgea```

### IBD
```{r ibd, fig.height=4, fig.width=4, cache=T, dpi=300, include=T}
