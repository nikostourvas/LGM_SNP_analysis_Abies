...
## PA
```{r fig.height=3, fig.width=4.2, include=T}
#bind dfs
...
## He
```{r uHe, fig.height=3, fig.width=4.2, include=T}
...
```{r Fst_p, fig.height=3, fig.width=4.2, include=T}
fst_comp <- as.data.frame(
      rbind(fst_ssr, fst_snp)
)
colnames(fst_comp) <- "fst"
fst_comp$marker <- c("11 SSR loci",
                     "140 SNP loci")
...
Και τα δύο συστήματα δεικτών παρουσιάζουν πολύ χαμηλές τιμές διαφοροποίησης των δύο πληθυσμών. Αν και οι SSR παρουσιάζουν αρκετά υψηλότερη τιμή, αυτό πιθανότατα οφείλεται στις διαφορές των δύο δεικτών (multiallelic, biallelic) και στην επιδραση που αυτές έχουν στο εύρος του των τιμών που μπορεί να λάβει το Fst.
...
```{r}
amova_SSR_df <- as.data.frame(
        amova_SSR$componentsofcovariance)
amova_SSR_df <- amova_SSR_df[-3,]
amova_SSR_df$component <- c("Between",
                            "Within")

# JUST USE KABLE

amova_SSR_df %>% 
        ggplot(aes(x=component, y=`%`)) +
        geom_bar(stat="identity")
```
...
```{r, fig.height=3, fig.width=4.5, include=T}
par_comp_p <- df_comp %>% 
