...
```{r missing, cache=F, fig.width=10, dpi=300, eval=F}
capture.output(
...
mis <- info %>% 
        ...
        ggtitle("Missing data frequency by cohort") +
...
```
Distribution of uHe is not normal, so 95% CIs of previews graph are probably misleading
...
```{r}
Ne_0.05_no_LD <- ldNe(obj_g, maf.threshold = 0.05, by.strata = F, ci = 0.95)
